...
class InvoiceTransactionTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final isFromInvoice = transaction.invoiceId != null && transaction.invoiceId!.isNotEmpty;
    
    _logger.info(
      '[TILE] Building transaction: ${transaction.description}, '
      'isFromInvoice: $isFromInvoice'
    );

    if (isFromInvoice) {
      _logger.warning(
        '[TILE] ✅ BLUE BG CONDITION MET! ID: ${transaction.id}'
      );
    } else {
      ...
      child: GestureDetector(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            color: isFromInvoice 
                ? const Color(0xFFE3E8F5) // Light blue background for invoice transactions
                : const Color(0xFFF5F5F5), // Subtle gray background for normal transactions
            borderRadius: BorderRadius.circular(8),
            border: Border(
              left: BorderSide(
                color: isFromInvoice
                    ? const Color(0xFF1976D2) // Vibrant blue for invoice transactions
                    : const Color(0xFF1976D2), // Blue left accent
                width: isFromInvoice ? 5 : 3, // Thicker border for invoice transactions
              ),
            ),
          ),
          child: Card(
